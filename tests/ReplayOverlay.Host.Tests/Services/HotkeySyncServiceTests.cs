...
public class HotkeySyncServiceTests
{
    [Theory]
    ...
    {
        Assert.Equal(expected, HotkeySyncService.MapObsKeyName(obsKey));
    }
    ...
    {
        Assert.Null(HotkeySyncService.MapObsKeyName("OBS_KEY_UNKNOWN_THING"));
    }
    ...
    public void MapObsKeyName_WithoutPrefix_StillWorks()
    {
        ...
        Assert.Equal("f1", HotkeySyncService.MapObsKeyName("F1"));
    }

    [Fact]
    public void MapObsKeyName_EmptyString_ReturnsNull()
    {
        Assert.Null(HotkeySyncService.MapObsKeyName(""));
    }
    ...
    public void ExtractHotkeyFromJson_ObsFormat_NumPlus()
    {
        ...
        var result = HotkeySyncService.ExtractHotkeyFromJson(json, "ReplayBuffer.Save");
    ...
    {
        var result = HotkeySyncService.ExtractHotkeyFromJson("not json", "ReplayBuffer.Save");
        Assert.Null(result);
    }

    [Fact]
    public void ReadReplayOverlayHotkey_ReturnsNullOrString()
    {
        // This reads from real OBS config; on CI or machines without OBS, returns null
        var result = HotkeySyncService.ReadReplayOverlayHotkey();
    ...
    {
        var result = HotkeySyncService.ReadObsRecordDirectory();
