...
public class ReplayFileServiceTests : IDisposable
{
    ...
    {
        _testDir = Path.Combine(Path.GetTempPath(), "ReplayOverlayTests_" + Guid.NewGuid().ToString("N")[..8]);
    ...
    public void Restart_SwitchesWatchFolder()
    {
        ...
        var dir2 = Path.Combine(Path.GetTempPath(), "ReplayOverlayTests2_" + Guid.NewGuid().ToString("N")[..8]);
